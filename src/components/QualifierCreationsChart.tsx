...
import { useQualifierCreations } from "@/lib/useQualifierCreations";
...
export const QualifierCreationsChart = memo(function QualifierCreationsChart({
  title = "Comportement de création — Qualificateurs",
}: { title?: string } = {}) {
  const now = new Date();
  ...
  const { data, error, isLoading } = useQualifierCreations(effectiveYm);
  ...
  // Build chart data: one row per day, one key per qualificateur + Total.
  ...
  const scopeLabel = scope === "today"
    ? `${monthLabel} · Aujourd'hui ${String(now.getDate()).padStart(2, "0")} ${MONTHS_FR[now.getMonth()]}: ${todayTotal} fiche${todayTotal > 1 ? "s" : ""}`
    : monthLabel;
  ...
  return (
    <Card className="shadow-elegant overflow-hidden">
      <CardHeader className="flex flex-row items-start sm:items-center justify-between gap-3 flex-wrap">
        <div>
          <CardTitle className="text-base xl:text-lg flex items-center gap-2">
            <Activity className="h-4 w-4 xl:h-5 xl:w-5 text-primary" />
            {title}
          </CardTitle>
          <CardDescription className="xl:text-sm">
            {scopeLabel} · {grandTotal} fiche{grandTotal > 1 ? "s" : ""} créée{grandTotal > 1 ? "s" : ""}
            {series.length > 0 && <> · {series.length} qualificateur{series.length > 1 ? "s" : ""}</>}
          </CardDescription>
        </div>
      ...
      <CardContent className="space-y-4">
        ...
        {data?.coverage && (
          <div className="rounded-md border border-border bg-muted/30 px-3 py-2 text-[11px] xl:text-xs text-muted-foreground flex flex-wrap gap-x-4 gap-y-1">
            <span><span className="font-semibold text-foreground">{data.coverage.totalProspectsInMonth}</span> fiches au total</span>
            <span>· <span className="text-emerald-600 dark:text-emerald-400 font-semibold">{data.coverage.fromActivityLog}</span> tracées (log)</span>
            <span>· <span className="text-amber-600 dark:text-amber-400 font-semibold">{data.coverage.fromAssigneeFallback}</span> attribuées via assignation (historique)</span>
            <span>· <span className="text-destructive font-semibold">{data.coverage.unattributed}</span> non-attribuables (assigné non-Qualificateur)</span>
          </div>
        )}

        {/* Per-qualificateur totals strip with toggling */}
        {series.length > 0 && (
        ...
        <div className="h-[28rem] xl:h-[34rem] rounded-xl border border-border bg-gradient-to-br from-muted/40 to-background p-3">
          {series.length === 0 && !isLoading ? (
            <div className="flex h-full items-center justify-center text-xs xl:text-sm text-muted-foreground">
              {scope === "today" ? "Aucune création de fiche aujourd'hui." : "Aucune création de fiche ce mois-ci."}
            </div>
          ) : allHidden ? (
            <div className="flex h-full items-center justify-center text-xs xl:text-sm text-muted-foreground">
              Sélectionnez au moins un qualificateur.
            </div>
          ) : (
                ...
                <ComposedChart data={chartData} margin={{ top: 16, right: 12, bottom: 4, left: -6 }} barCategoryGap="20%">
                  <defs>
                    {visibleSeries.map((s) => {
                      ...
                      return (
                        <linearGradient key={s.username} id={`qgrad-${s.username}`} x1="0" y1="0" x2="0" y2="1">
